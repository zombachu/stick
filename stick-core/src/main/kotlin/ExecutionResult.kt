...
sealed interface ParsingResult<T> : Result<T> {
    ...
    sealed interface SyntaxError<T> : ParsingResult<T>, Failure<T>
    ...
    sealed interface StateError<T> : ParsingResult<T>, Failure<T>
...
sealed class SenderValidationResult: Result<Unit> {
    open class Success internal constructor() : SenderValidationResult(), Result.Success<Unit> {
        override val value: Unit = Unit
    }
    open class Failure internal constructor(override val feedback: Feedback<out Tuple<String>>) : SenderValidationResult(), Result.Failure<Unit> {
